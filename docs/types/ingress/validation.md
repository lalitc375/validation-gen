...
| :--- | :--- | :--- | :--- |
| `metadata` | `metav1.ObjectMeta` | `+k8s:subfield(name=name)=+k8s:required`<br>`+k8s:subfield(name=name)=+k8s:format=k8s-long-name` | Ingress name is required and must be a DNS subdomain. |
...
| `spec.tls[].hosts` | `[]string` | `+k8s:optional`<br>`+k8s:eachVal=+k8s:format=k8s-long-name` | Optional list of hosts included in the TLS certificate. |
...
| Field Path | Go Type | Validation Tags | Reasoning / Notes |
| :--- | :--- | :--- | :--- |
| `service` | `*IngressServiceBackend` | `+k8s:optional` | References a service as a backend. Mutually exclusive with `resource`. |
| `service.name` | `string` | `+k8s:required`<br>`+k8s:format=k8s-long-name` | Mandatory name of the referenced service. |
| `service.port.name` | `string` | `+k8s:optional`<br>`+k8s:format=k8s-short-name` | Optional name of the port on the service. Mutually exclusive with `number`. |
| `service.port.number` | `int32` | `+k8s:optional`<br>`+k8s:minimum=1`<br>`+k8s:maximum=65535` | Optional numerical port. Mutually exclusive with `name`. |
| `resource` | `*api.TypedLocalObjectReference` | `+k8s:optional` | References another Kubernetes resource. Mutually exclusive with `service`. |
