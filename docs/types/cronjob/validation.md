...
| `spec.concurrencyPolicy` | `ConcurrencyPolicy` | `+k8s:required`<br>`+k8s:enum=["Allow", "Forbid", "Replace"]` | Mandatory policy for concurrent job executions. Defaults to `Allow`. |
