...
| `roleRef.apiGroup` | `string` | `+k8s:required`<br>`+k8s:enum=["rbac.authorization.k8s.io"]` | Mandatory API group for the role being referenced. |
| `roleRef.kind` | `string` | `+k8s:required`<br>`+k8s:enum=["Role", "ClusterRole"]` | Mandatory type of role being referenced. |
| `roleRef.name` | `string` | `+k8s:required` | Mandatory name of the role being referenced. |

### Subject

| Field Path | Go Type | Validation Tags | Reasoning / Notes |
| :--- | :--- | :--- | :--- |
| `kind` | `string` | `+k8s:required`<br>`+k8s:enum=["User", "Group", "ServiceAccount"]` | Mandatory type of subject being referenced. |
...
| `namespace` | `string` | `+k8s:optional` | Required for `ServiceAccount` if the subject is not in the same namespace as the RoleBinding. |
