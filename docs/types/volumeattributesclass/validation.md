...
| `parameters` | `map[string]string` | `+k8s:required`<br>`+k8s:eachKey=+k8s:minLength=1`<br>`+k8s:minItems=1`<br>`+k8s:immutable` | Mandatory volume attributes defined by the CSI driver. Must contain at least one key/value pair. Maximum 512 parameters. Immutable after creation. |
