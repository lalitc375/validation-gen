...
| `spec.behavior.scaleUp.selectPolicy` | `*ScalingPolicySelect` | `+k8s:optional`<br>`+k8s:enum=["Max", "Min", "Disabled"]` | Optional policy selection for scaling up. |
| `spec.behavior.scaleDown.selectPolicy` | `*ScalingPolicySelect` | `+k8s:optional`<br>`+k8s:enum=["Max", "Min", "Disabled"]` | Optional policy selection for scaling down. |
...
| `spec.behavior.scaleUp.policies[].type` | `HPAScalingPolicyType` | `+k8s:required`<br>`+k8s:enum=["Pods", "Percent"]` | Mandatory policy type. |
