...
| `spec.fsGroupPolicy` | `*FSGroupPolicy` | `+k8s:optional`<br>`+k8s:enum=["ReadWriteOnceWithFSType", "File", "None"]` | Controls if Kubernetes should modify volume ownership and permissions. |
| `spec.volumeLifecycleModes` | `[]VolumeLifecycleMode` | `+k8s:optional`<br>`+k8s:immutable`<br>`+k8s:eachVal=+k8s:enum=["Persistent", "Ephemeral"]` | Supported volume modes. Immutable after creation. |
