...
| `spec.podManagementPolicy` | `PodManagementPolicyType` | `+k8s:optional`<br> | Optional policy for creating pods. Defaults to `OrderedReady`. |
| `spec.updateStrategy.type` | `StatefulSetUpdateStrategyType` | `+k8s:optional`<br> | Optional strategy for updates. Defaults to `RollingUpdate`. |
...
| `spec.volumeClaimTemplates` | `[]PersistentVolumeClaim` | `+k8s:optional`<br>`+k8s:listType=atomic` | Optional list of claims that pods are allowed to reference. |
...
| Field Path | Go Type | Validation Tags | Reasoning / Notes |
| :--- | :--- | :--- | :--- |
| `conditions` | `[]StatefulSetCondition` | `+k8s:optional`<br>`+k8s:listType=map`<br>`+k8s:listMapKey=type` | Represents the latest available observations of a statefulset's current state. |
...
| :--- | :--- |
| `PodManagementPolicyType` | `+k8s:enum` |
| `StatefulSetUpdateStrategyType` | `+k8s:enum` |
