...
| Field Path | Go Type | Validation Tags | Reasoning / Notes |
| :--- | :--- | :--- | :--- |
| `capacity` | `ResourceList` | `+k8s:optional`<br>`+k8s:eachKey=+k8s:format=k8s-container-resource-name` | Total resources of the node. Each value must be non-negative. |
| `allocatable` | `ResourceList` | `+k8s:optional`<br>`+k8s:eachKey=+k8s:format=k8s-container-resource-name` | Resources available for scheduling. Each value must be non-negative. |
